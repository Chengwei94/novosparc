...
class Tissue:
    ...
    def setup_smooth_costs(
        ...
        (
            ...
            self.costs["locations"],
        ) = novosparc.rc.setup_for_OT_reconstruction(
            dge_rep,
            self.locations,
    ...
    def setup_linear_cost(
        self,
        markers_to_use=None,
        atlas_matrix=None,
        markers_metric="euclidean",
        markers_metric_p=2,
    ):
        """
        ...
        """
        self.atlas_matrix = (
            atlas_matrix if atlas_matrix is not None else self.atlas_matrix
        )
        self.markers_to_use = (
            markers_to_use if markers_to_use is not None else self.markers_to_use
        )

        cell_expression = self.dge[:, self.markers_to_use] / np.amax(
            self.dge[:, self.markers_to_use]
        )
        atlas_expression = self.atlas_matrix / np.amax(self.atlas_matrix)

        self.costs["markers"] = cdist(
            cell_expression, atlas_expression, metric=markers_metric, p=markers_metric_p
        )
    ...
    def reconstruct(
        ...
        if verbose:
            print(
                "Reconstructing spatial information with",
                self.num_markers,
                "markers:",
                self.num_cells,
                "cells and",
        ...
        p_locations_c, p_expression_c = novosparc.rc.create_space_distributions(
            self.num_locations, self.num_cells
        )
        ...
        print("Starting wasserstein")

        while first_pass and (epsilon < max_epsilon):
        ...
        print("Ending wasserstein")

        self.epsilon = epsilon
        self.gw = gw

        sdge = self.dge.T.dot(gw[0])
        self.sdge = sdge
    ...
    def calculate_spatially_informative_genes(self, selected_genes=None, n_neighbors=8):
        ...
        if selected_genes is not None:
            ...
            gene_indices = (
                pd.DataFrame(np.arange(self.num_genes), index=self.gene_names)[0]
                .loc[selected_genes]
                .values
            )
        ...
        mI, pvals = novosparc.an.get_moran_pvals(
            sdge.T, self.locations, n_neighbors=n_neighbors
        )
        mI = np.array(mI)
