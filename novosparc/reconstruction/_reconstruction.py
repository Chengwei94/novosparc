...
def setup_for_OT_reconstruction(
    dge,
    locations,
    ...
    expression_metric="minkowski",
    expression_metric_p=2,
    verbose=True,
):
    ...
    A_locations = kneighbors_graph(
        locations,
        num_neighbors_target,
        mode="connectivity",
        include_self=True,
        metric=locations_metric,
        p=locations_metric_p,
    )
    sp_locations = dijkstra(
        csgraph=csr_matrix(A_locations), directed=False, return_predecessors=False
    )
    sp_locations_max = np.nanmax(sp_locations[sp_locations != np.inf])
    sp_locations[
        sp_locations > sp_locations_max
    ] = sp_locations_max  # set threshold for shortest paths

    num_neighbors_source = num_neighbors_source  # number of neighbors for nearest neighbors graph at source
    A_expression = kneighbors_graph(
        dge,
        num_neighbors_source,
        mode="connectivity",
        include_self=True,
        metric=expression_metric,
        p=expression_metric_p,
    )
    sp_expression = dijkstra(
        csgraph=csr_matrix(A_expression), directed=False, return_predecessors=False
    )
    sp_expression_max = np.nanmax(sp_expression[sp_expression != np.inf])
    sp_expression[
        sp_expression > sp_expression_max
    ] = sp_expression_max  # set threshold for shortest paths
...
def find_spatial_archetypes(num_clusters, sdge):
    ...
    clusters = hierarchy.fcluster(
        hierarchy.ward(sdge), num_clusters, criterion="maxclust"
    )
    ...
    for gene in range(len(sdge)):
        gene_corrs = np.append(
            gene_corrs, pearsonr(sdge[gene, :], archetypes[clusters[gene] - 1, :])[0]
        )
    print("done")
...
def get_genes_from_spatial_archetype(
    sdge, gene_names, archetypes, archetype, pval_threshold=0
):
    ...
    for g in range(len(sdge)):
        all_corrs = np.append(
            all_corrs, pearsonr(sdge[g, :], archetypes[archetype, :])[0]
        )
        all_corrs_p = np.append(
            all_corrs_p, pearsonr(sdge[g, :], archetypes[archetype, :])[1]
        )
...
def find_spatially_related_genes(sdge, gene_names, archetypes, gene, pval_threshold=0):
    ...
    for archetype in range(len(archetypes)):
        arch_corrs = np.append(
            arch_corrs, pearsonr(sdge[gene, :], archetypes[archetype, :])[0]
        )
    if np.max(arch_corrs) < 0.7:
        print(
            "No significant correlation between the gene and the spatial archetypes was found."
        )
        return None
    archetype = np.argmax(arch_corrs)

    return get_genes_from_spatial_archetype(
        sdge, gene_names, archetypes, archetype, pval_threshold=pval_threshold
    )
